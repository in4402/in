...
<c:set var="content" value="${fn:replace(vo.content, newLine, '<br>') }" />
	...
	<div class="divForm">
		...
		<div>
			<span class="sp1">첨부파일정보</span> 
			<span>
				<c:if test="${!empty vo.fileName }">
					<a href="<c:url value='/reBoard/download.do?no=${vo.no }&fileName=${vo.fileName }'/>">${fileInfo}</a> 
						다운 : ${vo.downCount}
				</c:if>
			</span>
		</div>
		...
		<div class="center">
        	...
        	<a href='<c:url value="/reBoard/delete.do?no=${param.no}"/>'>삭제</a> |
